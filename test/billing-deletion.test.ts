...
function subscription(
  ...
  return {
    ...
    product: {
      id: 'product-monthly',
      price: { currency: 'USD', price_type: 'fixed', amount: '9.00' },
...
describe('billing-safe deletion provider confirmation', () => {
  ...
  it('does not contact Bachs when no provider subscription exists', async () => {
    ...
    await expect(confirmSubscriptionWillNotRenew(
      { workspaceId: 'workspace-free', subscriptionId: null, invoiceReference: null },
  ...
  it.each([
    ...
    const result = await confirmSubscriptionWillNotRenew(
      { workspaceId: 'workspace-1', subscriptionId: current.id, invoiceReference: 'invoice-1' },
  ...
  it('cancels an active subscription with a stable idempotency key', async () => {
    ...
    await confirmSubscriptionWillNotRenew(
      { workspaceId: 'workspace-1', subscriptionId: 'sub-1', invoiceReference: 'invoice-1' },
      dependencies
    )
    await confirmSubscriptionWillNotRenew(
      { workspaceId: 'workspace-1', subscriptionId: 'sub-1', invoiceReference: 'invoice-1' },
  ...
  it('rejects a mismatched subscription returned by either provider call', async () => {
    await expect(confirmSubscriptionWillNotRenew(
      { workspaceId: 'workspace-1', subscriptionId: 'sub-expected', invoiceReference: 'invoice-1' },
      {
  ...
  it('rejects a provider subscription bound to another workspace or invoice', async () => {
    await expect(confirmSubscriptionWillNotRenew(
      { workspaceId: 'workspace-1', subscriptionId: 'sub-1', invoiceReference: 'invoice-1' },
      {
        getSubscription: vi.fn().mockResolvedValue(subscription('sub-1', 'active', false, 'workspace-2')),
        cancelSubscription: vi.fn()
      }
    )).rejects.toMatchObject({ statusCode: 502 })
    ...
    await expect(confirmSubscriptionWillNotRenew(
      { workspaceId: 'workspace-1', subscriptionId: 'sub-1', invoiceReference: 'invoice-1' },
      {
        getSubscription: vi.fn().mockResolvedValue(subscription('sub-1', 'active', false, 'workspace-1', 'invoice-2')),
        cancelSubscription: vi.fn()
      }
    )).rejects.toMatchObject({ statusCode: 502 })
  })
