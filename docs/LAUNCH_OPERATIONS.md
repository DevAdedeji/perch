...
- Only one unresolved checkout lineage can exist per workspace. A database constraint and creation lease serialize competing tabs before provider checkout creation, and the stable workspace invoice reference is also sent as Bachs' idempotency key. Verified email is required before checkout. If an unexpected second subscription lineage is ever observed, Perch does not grant it and sends an idempotent cancellation request after validating the canonical workspace and invoice metadata.
