...
- Configure and verify a signed Resend webhook before enabling visitor reply delivery so bounces and complaints create durable hashed-email suppressions. Set its endpoint to `/api/webhooks/resend`, subscribe only to `email.bounced` and `email.complained`, and store the endpoint signing secret as `RESEND_WEBHOOK_SECRET`. Production startup rejects visitor reply delivery when that secret is missing or weak.
...
- Attachment records bind every new upload to its workspace and uploader or visitor before it can be used. Abandoned uploads are collected after 24 hours; replacement and deletion cleanup retries five times and then appears in the admin cleanup queue for manual retry. Monitor `attachment_cleanup.failed` and the oldest outstanding item in admin metrics.
- Workspace and account deletion freeze new checkout creation before provider cancellation. A paid workspace is deleted only after Bachs confirms the exact workspace subscription cannot renew. Minimal cancellation evidence survives deletion without customer identity or payment amounts. If Bachs or the database cannot confirm the operation, Perch keeps the data and shows a recoverable error.
