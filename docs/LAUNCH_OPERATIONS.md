...
- Keep `PERCH_BILLING_CHECKOUT_ENABLED=false` until Bachs authoritatively documents or supplies fixtures for its canonical recurring product/subscription response fields and the full sandbox checkout, payment, renewal, cancellation, failure, and reconciliation journeys pass. The application currently enforces this provider-contract prerequisite in addition to the environment flag, so new checkout cannot be opened by configuration alone. Turning the flag off does not disable existing entitlements, cancellation, or signed Bachs webhook processing.
