...
- Billing reconciliation is independent of the checkout gate. A database-backed sweep checks at most five due workspaces per minute, and each claimed workspace reads only its current checkout and known subscription (two canonical resources maximum). Each safe GET has at most three attempts, so one reconciliation has a strict six-request provider cap. Failed jobs back off from 5 minutes to 6 hours, stop after six consecutive failures, recover stale 10-minute claims, and can be retried by an admin from **Plans & billing**.
- Manual, automatic, and webhook-triggered reconciliation share one workspace claim. A request arriving during a claim requeues the workspace; claim tokens prevent a reclaimed/older worker from completing over a newer one. A subscription webhook needs one canonical read to discover its workspace. Perch applies that snapshot only when the workspace's last completed check predates the read; otherwise it requeues instead of allowing a late response to overwrite newer state. This keeps the webhook path to one subscription read plus one checkout read even though Bachs does not expose a provider version in the current response schema.
- Perch never grants Pro from a redirect or webhook body. It refetches Bachs and requires the exact workspace, invoice reference, product plan, amount, currency, and interval, plus a future provider-bounded subscription period. A confirmed checkout without a known subscription ID remains paid-but-Free until a signed subscription event supplies that ID. The current Bachs client has no documented subscription-discovery endpoint, so do not invent one; investigate the failed/dead-letter webhook in Bachs support tools instead.
