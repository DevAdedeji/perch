...
- Generate `VISITOR_REPLY_SECRET` independently with at least 32 random characters. Rotating it immediately invalidates outstanding visitor return and unsubscribe links.
- Keep `VISITOR_REPLY_EMAIL_DELIVERY_ENABLED=false` until visitor-email processing through Resend is explicitly approved, the scheduled delivery worker is registered, and the staging journey below passes. The worker utility also refuses its production transport while this gate is false; injected test transports remain available for safe automated verification.
